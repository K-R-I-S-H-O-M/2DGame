...
public class HelpMethods {
    public static boolean CanMoveHere(float x, float y, float width, float height, int[][] lvlData) {
        ...
        {
            if (!IsSolid(x + width, y + height, lvlData))// bottom left
            {
    ...
    public static float GetEntityXPosNextToWall(Rectangle2D.Float hitbox, float xSpeed) {
        int currentTiles = (int) (hitbox.x / Game.TILES_SIZE);
        if (xSpeed > 0) {
            // right
            int tileXPos = currentTiles * Game.TILES_SIZE;
            int xOffset = (int) (Game.TILES_SIZE - hitbox.width);
            return tileXPos + xOffset - 1;
        } else {
            // Left
            return currentTiles * Game.TILES_SIZE;
        }
    }
    ...
    public static boolean IsAllTilesWalkable(int xStart, int xEnd, int y, int[][] lvlData) {
        if(IsAllTilesClear(xStart, xEnd, y, lvlData))
            for (int i = 0; i < xEnd - xStart; i++) {
                ...
                if (!IsTileSolid(xStart, y + 1, lvlData)) {
                    return false;
                }
            }
        return true;
