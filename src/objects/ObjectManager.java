...
public class ObjectManager {
    ...
    public void applyEffectToPlayer(Potion p) {
        if (p.getObjType() == RED_POTION) {
            playing.getPlayer().changeHealth(RED_POTION_VLAUE);
        } else {
            playing.getPlayer().changePower(BLUE_POTION_VLAUE);
        }
    }
    ...
    private void shootCannon(Cannon c) {
        ...
        projectiles.add(new Projectiles((int)(c.getHitbox().x), (int)(c.getHitbox().y), dir));
    }

    private boolean isPlayerInFrontOfCannon(Cannon c, Player player) {
        if(c.getObjType() == CANNON_LEFT){
            if(c.getHitbox().x > player.getHitbox().getX())
                return true;  
        }
        else if(c.getHitbox().x < player.getHitbox().getX()){
            return true;
        }
        return false;
    }
